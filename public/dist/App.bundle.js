...
function ajaxHeart(e) {
  ...
  _axios2.default.post(this.action).then(function (res) {
    var isHearted = _this.heart.classList.toggle('heart__button--hearted'); // accessing form's name attribute
...
function loadPlaces(map) {
  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    //create bounds for our map. so our zoom level is optimized for any markers we could have
    ...
    // when someone clicks on our marker, show details
    ...
    // zoom the map in to fit our bounds
...
function typeAhead(search) {
  ...
  var searchResultsCount = searchResults.children.length;
  var activeSearchResult = search.querySelector('.search__result--active');
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class=\'search__result\'>No Results for ' + _this.value + ' found. Please try another search term!</div>');
    }).catch(function (err) {
...
(0, _autocomplete2.default)((0, _bling.$)('#address'), (0, _bling.$)('#lat'), (0, _bling.$)('#lng')); // selecting the inputs with Bling and sending them to autocomplete functions. same as document.querySelector('#address')
...
heartForms.on('submit', _heart2.default); // with bling, you can listen to events on multiple nodes in NodeList
